...
XREAD   LDA #RDCMND     ; SETUP READ SECTOR COMMAND
        STA COMREG      ; ISSUE READ COMMAND 
        LBSR XDEL28      ; DELAY 
        CLRB            ; GET SECTOR LENGTH (=256) 
XREAD3   LDA COMREG      ; GET WD STATUS 
        BITA #DRQ       ; CHECK FOR DATA 
        BNE XREAD5       ; BRANCH IF DATA PRESENT 
        BITA #BUSY      ; CHECK IF BUSY 
        BNE XREAD3       ; LOOP IF SO 
        TFR A,B         ; ERROR IF NOT 
        BRA XREAD6 
XREAD5   LDA DATREG      ; GET DATA BYTE 
        STA 0,X+        ; PUT IN MEMORY 
        DECB            ; DEC THE COUNTER 
        BNE XREAD3       ; LOOP TIL DONE 
        BSR XWAIT        ; WAIT TIL WD IS FINISHED 
XREAD6   BITB #RDMSK     ; MASK ERRORS 
        RTS             ; RETURN
...
XRESTORE LDA #RSCMND     ; SETUP RESTORE COMMAND 
        STA COMREG      ; ISSUE RESTORE COMMAND 
        BSR XDEL28       ; DELAY 
        LBSR XWAIT       ; WAIT TIL WD IS FINISHED 
